...
export function MessengersFilterDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  const onSubmit = (data: FilterFormValues) => {
    console.log('Filtros de mensageiros aplicados:', data);
    onOpenChange(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Filtros de Mensageiros</DialogTitle>
          <DialogDescription>
            Refine sua busca por mensageiros com os filtros abaixo.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
            ...
            <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-3">
                    <FormLabel>Status do Mensageiro</FormLabel>
                    <FormControl>
                      <RadioGroup
